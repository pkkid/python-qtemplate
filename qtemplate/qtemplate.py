...
from qtemplate import base, helpers, log, utils
...
class QTemplateWidget(QtWidgets.QWidget):
    """ My interpreation of a simpler Qt Template language. """
    TMPL = None             # Filepath of tmpl file to load
    TMPLSTR = None          # String template (if not using a file)
    verbose = False         # Set True to log verbose messages
    globalcontext = {}      # Global template tags and values
    ...
    def __new__(cls, *args, **kwargs):
        """ Populate the global context if not done already. """
        if 'Qt' not in cls.globalcontext:
            cls.globalcontext['Qt'] = QtCore.Qt
            for module in (QtGui, QtWidgets):
                members = dict(inspect.getmembers(module, inspect.isclass))
                cls.globalcontext.update({k:v for k,v in members.items()})
            for helper in helpers.__all__:
                cls.globalcontext[helper.__name__] = helper
    ...
    def _tagQObject(self, elem, parent, context, indent):
        ...
        if elem.tag in self.globalcontext:
            qcls = utils.rget(self.globalcontext, elem.tag)
